	...
	function continued()
	{
		...
		connection.query('SELECT username,salt FROM Users WHERE BINARY email = ?', [email], function(err,rows,fields)
		{
			if(err){
				...
				global.renderPage('login', {message: {type:'error', content: res.i18n.__('server.genericErr')}})(req,res);
				connection.end();
			}
			else if(rows.length === 0){
				global.log('Email not found:', email);
				global.renderPage('login', {message: {type:'error', content: res.i18n.__('server.loginErr')}})(req,res);
				connection.end();
			}
			else
			{
